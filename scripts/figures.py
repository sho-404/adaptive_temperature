    ...
    for ax, m in zip(axes, MODELS):
        ...
        for pos, col, lab in [("mid", CAT[0], "mid-chain (50%)"),
            ...
            ax.plot(np.array(taps) / taps[-1], ys, "-o", ms=3, lw=1.4,
                    color=col, label=lab)
        ax.axhline(0.5, color=MUTED, lw=0.8, ls=":")
        ax.set_title(f"{LABEL[m]} ({n_layers} layers)", pad=16)
        ax.set_xlabel("relative layer depth")
        ax.set_ylim(0.45, 0.88)
        despine(ax)
        top = ax.secondary_xaxis("top")
        top.set_xticks(np.array(taps) / taps[-1])
        top.set_xticklabels([str(t) for t in taps])
        top.tick_params(axis="x", labelsize=6, colors=MUTED, length=2)
        top.spines["top"].set_visible(False)
        top.set_xlabel("layer index", fontsize=6.5, color=MUTED, labelpad=2)
